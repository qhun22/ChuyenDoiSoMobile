...
export default function ProfilePage() {
  ...
  // Loading States cho danh sách (3s loading pattern)
  const [isLoadingAddresses, setIsLoadingAddresses] = useState<boolean>(true);
  const [isLoadingPasswordHistory, setIsLoadingPasswordHistory] = useState<boolean>(true);
  ...
  useEffect(() => {
    ...
    const sortDefaultFirst = (list: Address[]) => {
      const def = list.filter((a) => a.isDefault);
      const nonDef = list.filter((a) => !a.isDefault);
      return [...def, ...nonDef];
    };
    ...
    if (cached) {
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          setAddresses(sortDefaultFirst(parsed));
        }
      } catch {
        // Bỏ qua lỗi parse
      }
    ...
    fetch('/api/addresses', {
      ...
      .then((data) => {
        if (data.addresses && Array.isArray(data.addresses)) {
          const sorted = sortDefaultFirst(data.addresses);
          setAddresses(sorted);
  ...
  useEffect(() => {
    ...
    setIsLoadingPasswordHistory(true);
    const startTime = Date.now();
  ...
  // 2. Tải danh sách Xã/Phường theo Tỉnh được chọn (có hiệu ứng load 3s)
  useEffect(() => {
  ...
  const handleAddressSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const isDefault = isFormDefault || form.get('isDefault') === 'on';
    ...
    if (!phoneRegex.test(cleanPhone)) {
      toast.error('Số điện thoại không đúng định dạng 10 chữ số (Ví dụ: 0975123456)!');
      return;
    }
    ...
    if (!communeName || communeName.startsWith('--') || communeName.includes('Đang tải')) {
      toast.error('Vui lòng chọn Phường/Xã!');
      return;
    }

    if (!detail) {
      toast.error('Vui lòng nhập địa chỉ chi tiết (Thôn, Tổ dân phố, Số nhà, Đường)!');
      return;
    }
  ...
  const setDefaultAddress = async (addressId: number) => {
    ...
    try {
      ...
      setAddresses((currentAddresses) => {
        ...
        if (!target) return currentAddresses;
        const targetUpdated = { ...target, isDefault: true };
        const others = currentAddresses
          .filter((a) => a.id !== addressId)
          .map((a) => ({ ...a, isDefault: false }));
        const sorted = [targetUpdated, ...others];
        localStorage.setItem('cached_addresses', JSON.stringify(sorted));
        return sorted;
      });
      setAddressPage(1);
      toast.success('Đã đặt làm địa chỉ mặc định (đưa lên đầu danh sách)');
    } catch {
      // Fallback local
      setAddresses((currentAddresses) => {
        const target = currentAddresses.find((a) => a.id === addressId);
          ...
          <div style={{ animation: 'fadeInTab 0.28s ease' }} className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 min-h-[460px]">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch h-full">
              ...
              <div className="lg:col-span-6 flex flex-col justify-between h-full">
                ...
                <form className="flex-1 flex flex-col justify-between space-y-3 pt-2" onSubmit={handleAddressSubmit} noValidate>
                  {/* Họ tên & SĐT */}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {isLoadingCommunes && (
                          <span className="inline-flex items-center gap-1 text-[10px] text-[#d70018] font-medium animate-pulse">
                      ...
                      <div className="relative">
                        <select
                          ...
                          onFocus={(e) => { e.currentTarget.style.animation = 'inputFocusGlow 0.3s ease forwards'; }}
                          onBlur={(e) => { e.currentTarget.style.animation = ''; }}
                          className={`w-full rounded-lg border px-3.5 py-2.5 text-xs text-slate-700 cursor-pointer transition-all duration-200 ${
                            isLoadingCommunes
                              ? 'border-red-300 bg-red-50/40 text-slate-600 animate-pulse'
                              : 'border-slate-200 bg-slate-50/50 focus:outline-none focus:border-[#d70018]'
                          } disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400`}
                          disabled={isLoadingCommunes || (!selectedProvinceId && communes.length === 0)}
                        >
                          <option value="">
                            {isLoadingCommunes
                        ...
                        {isLoadingCommunes && (
                          <div className="absolute right-8 top-1/2 -translate-y-1/2 pointer-events-none">
                            <svg className="animate-spin w-3.5 h-3.5 text-[#d70018]" fill="none" viewBox="0 0 24 24">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 mb-1">
                      Địa chỉ chi tiết (Thôn, Tổ Dân Phố, Số nhà...) <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text"
                      name="detail"
                      placeholder="Ví dụ: Số 123 Đường ABC, TDP 4..."
                      onFocus={(e) => { e.currentTarget.style.animation = 'inputFocusGlow 0.3s ease forwards'; }}
                      onBlur={(e) => { e.currentTarget.style.animation = ''; }}
                      className="w-full px-3.5 py-2.5 rounded-lg border border-slate-200 text-xs focus:outline-none focus:border-[#d70018] bg-slate-50/50 transition focus:scale-[1.01]"
                    />
                  ...
                  {/* Checkbox Đặt làm mặc định tương tác mượt */}
                  <div 
                    onClick={() => setIsFormDefault(!isFormDefault)}
                    className="flex items-center gap-2.5 pt-1 cursor-pointer select-none group w-fit"
                  >
                    <input
                      type="hidden"
                      name="isDefault"
                      value={isFormDefault ? 'on' : 'off'}
                    />
                    <div className={`w-4 h-4 rounded border flex items-center justify-center transition-all duration-200 transform ${
                      isFormDefault 
                        ? 'bg-[#d70018] border-[#d70018] shadow-xs scale-105' 
                        : 'border-slate-300 bg-white group-hover:border-slate-400'
                    }`}>
                      {isFormDefault && (
                        <svg 
                          style={{ animation: 'checkmarkPop 0.22s ease-out' }}
                    ...
                    <span className={`text-xs transition-colors duration-150 ${isFormDefault ? 'text-slate-900 font-semibold' : 'text-slate-600 group-hover:text-slate-800'}`}>
              ...
              <div className="lg:col-span-6 flex flex-col justify-between h-full">
                ...
                <div className="flex-1 w-full min-h-[390px] p-0 flex flex-col justify-between items-stretch overflow-visible mt-2">
                  {isLoadingAddresses ? (
                    <div className="w-full h-full flex flex-col justify-between gap-3 overflow-visible">
                      <div className="space-y-2.5 pt-2 pb-2 px-1">
                        {[1, 2, 3].map((i) => (
                          <div key={i} className="rounded-lg border border-slate-100 bg-white p-3 space-y-2.5 animate-pulse shadow-xs">
                      ...
                      <div className="text-center text-[11px] text-slate-400 font-medium py-3 flex items-center justify-center gap-1.5">
                        <svg className="animate-spin w-3.5 h-3.5 text-[#d70018]" fill="none" viewBox="0 0 24 24">
                          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" />
                        </svg>
                    ...
                    <div className="w-full flex flex-col items-center justify-center text-center p-4 my-auto">
                    ...
                    <div className="w-full h-full flex flex-col justify-between gap-3 overflow-visible">
                      <div className="space-y-2.5 pt-2 pb-2 px-1 overflow-visible">
                        {addresses.slice((addressPage - 1) * 3, addressPage * 3).map((address) => (
                          <div
                            key={address.id}
                            style={{ animation: 'slideInAddress 0.3s ease' }}
                            className={`rounded-lg border bg-white p-3 text-xs text-slate-700 shadow-xs hover:shadow-md hover:-translate-y-[2px] transition-all duration-200 relative ${
                              address.isDefault ? 'border-[#d70018] ring-1 ring-[#d70018]/30 shadow-red-500/5' : 'border-slate-200 hover:border-slate-300'
                            }`}
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div>
                                ...
                                <div className="mt-2 flex items-center gap-2">
                                  {address.isDefault ? (
                                    <span className="rounded-full bg-[#d70018] px-2 py-1 text-[10px] font-bold text-white shadow-xs">
                                      Đang mặc định
                                    </span>
                                  ) : (
                                    <button
                                      ...
                                      className="rounded-md border border-[#d70018] px-2 py-1 text-[10px] font-semibold text-[#d70018] hover:bg-red-50 active:scale-95 transition-all duration-150 cursor-pointer disabled:opacity-50"
                                    >
                                  ...
                                  <button
                                    ...
                                    className="rounded-md border border-slate-200 px-2 py-1 text-[10px] font-semibold text-slate-600 hover:bg-red-50 hover:border-red-200 hover:text-red-600 active:scale-95 transition-all duration-150 cursor-pointer"
                                  >
                                    Xóa
                                  </button>
                                </div>
                      ...
                      <div className="flex items-center justify-center gap-2 py-3 px-2 overflow-visible">
                        <button
                          type="button"
                          disabled={addressPage === 1}
                          onClick={() => setAddressPage((page) => page - 1)}
                          className="h-8 w-8 sm:h-9 sm:w-9 rounded-full border border-slate-200 text-slate-500 hover:scale-110 active:scale-90 transition-all duration-150 disabled:opacity-40 disabled:hover:scale-100 cursor-pointer flex items-center justify-center bg-white shadow-2xs"
                        >
                          ‹
                        </button>
                        {paginationItems.map((page) => (
                          <button
                            ...
                            className={`h-8 w-8 sm:h-9 sm:w-9 rounded-full text-xs font-bold hover:scale-110 active:scale-90 transition-all duration-150 cursor-pointer flex items-center justify-center ${
                              addressPage === page ? 'bg-[#d70018] text-white shadow-md' : 'border border-slate-200 text-slate-700 hover:border-slate-300 bg-white'
                            }`}
                          >
                            {page}
                          </button>
          ...
          <div style={{ animation: 'fadeInTab 0.28s ease' }} className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 min-h-[460px]">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch h-full">
              <div className="lg:col-span-6 flex flex-col justify-between h-full">
                ...
                <form className="flex-1 flex flex-col justify-between space-y-3 pt-2" onSubmit={handlePasswordSubmit} noValidate>
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 mb-1">
              ...
              <div className="lg:col-span-6 flex flex-col justify-between h-full">
                ...
                <div className="flex-1 w-full min-h-[390px] p-0 flex flex-col justify-between items-stretch overflow-visible mt-2">
                  {isLoadingPasswordHistory ? (
                    <div className="w-full h-full flex flex-col justify-between gap-3 overflow-visible">
                      <div className="space-y-2.5 pt-2 pb-2 px-1">
                        {[1, 2, 3, 4].map((i) => (
                          <div key={i} className="rounded-lg border border-slate-100 bg-white p-3 space-y-2 animate-pulse shadow-xs">
                      ...
                      <div className="space-y-2.5 pt-2 pb-2 px-1 overflow-visible">
                        {passwordHistory.slice((passwordPage - 1) * 4, passwordPage * 4).map((entry) => (
                          <div
                            ...
                            className="rounded-lg border border-slate-200 bg-white p-3 text-xs text-slate-700 shadow-xs hover:shadow-md hover:-translate-y-[2px] transition-all duration-200"
                          >
                      ...
                      <div className="flex items-center justify-center gap-2 py-3 px-2 overflow-visible">
                        ...
                        {passwordPaginationItems.map((page) => (
                          <button
                            ...
                            className={`h-8 w-8 sm:h-9 sm:w-9 rounded-full text-xs font-bold hover:scale-110 active:scale-90 transition-all duration-150 cursor-pointer flex items-center justify-center ${
                              passwordPage === page ? 'bg-[#d70018] text-white shadow-md' : 'border border-slate-200 text-slate-700 hover:border-slate-300 bg-white'
                            }`}
                          >
                            {page}
                          </button>
